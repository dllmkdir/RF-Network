...
void TIM2_IRQHandler(void){//Handler for TIM2 interruption
	// Prepare data packet
		...
		/*
		b = ( GPIOA->IDR & (1<<0) )>>0;//Panic button GPIO PA0
			  if(b){
				panicButtonFlag=1;
			  }else{
				panicButtonFlag=0;
			  }
		c = ( GPIOA->IDR & (1<<1) )>>1;//Flame Detection GPIO PA1
			  if(c){
				flameFlag=1;
			  }else{
				flameFlag=0;
			  }
		d = ( GPIOA->IDR & (1<<2) )>>2;//door sensor GPIO PA2
			  if(d){
				doorFlag=1;
			  }else{
				doorFlag=0;
			  }
		e = ( GPIOA->IDR & (1<<3) )>>3;//door sensor GPIO PA3
			  if(e){
				movFlag=1;
			  }else{
				movFlag=0;
